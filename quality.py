...
def quality_score(tp, tn, fp, fn) -> float:
    return (tp + tn) / (tp + tn + 10 * fp + fn)

def compute_quality_for_corpus(corpus_dir) -> (float, confmat.BinaryConfusionMatrix):
    ...
    return quality_score(**matrix.as_dict()), matrix
